...
#############################        
    def Calculate(self):
        from ShaoWangV3 import ShaoWang
        from numpy import shape
        import time
        start2 = time.time()
        V0C=self.pho.Potential+self.pho.w0
        V0X=self.exc.Potential+self.exc.w0
        EmC=self.pho.mass
        EmX=self.exc.mass
        ...
        if (shape(self.pho.Psi0)==shape(self.exc.Psi0) and shape(self.pho.Psi0)==shape(self.x) 
            ...
            self.pho.Psi,self.exc.Psi=ShaoWang(self.pho.Psi0,self.exc.Psi0,V0C,V0X,self.pho.pump,EmC,
