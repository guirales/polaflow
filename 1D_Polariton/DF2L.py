...
def DF2L2R(Psi0c,Psi0x,V0c,V0x,f,a0c,a0x,gR,g,gammaC,gammaX,X,T0,puntos=2,precision=10):
    ...
    def Sc(Psic,Psix,F):
        return A1c*Psic+dot(alphaC,Psic)-1.0j*gR*Psix-1.0j*F-gammaC*Psic
    def Sx(Psic,Psix):
        return A1x*Psix+dot(alphaX,Psix)-1.0j*g*abs(Psix)**2*Psix-1.0j*gR*Psic-gammaX*Psix
#          
...
#    d=Nx if puntos<2 else int(puntos)   
    ...
    A1c=1.0j*C0*a0c/(2*Dx**2)-1.0j*V0c##        
    ...
    A1x=1.0j*C0*a0x/(2*Dx**2)#-1.0j*V0x#
################# Barra de estado
    nl=int((Nt+1)/10)
    print "[__h__m__s|"+(10*"#")+"]"
    sys.stdout.write("[")
